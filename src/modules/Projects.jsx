...
function ProjectCard({ project, onEdit, onDelete }) {
  return (
    <div className="card" style={{ marginBottom: 12 }}>
      ...
      {project.tags?.length > 0 && <div style={{ display: 'flex', gap: 6, marginTop: 12, flexWrap: 'wrap' }}>{project.tags.map(t => <span key={t} className="tag">{t}</span>)}</div>}
    </div>
  )
}
